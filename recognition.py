...
class recognition:
    ...
    def compare(self,img):
        ...
        # print(person_norm)
        wzeros_norm = F.normalize(self.wzeros, dim=0)
        # print(wzeros_norm)
        cls = torch.matmul(person_norm, wzeros_norm)
...
if __name__ == '__main__':
    ...
    name = [("_") for i in range(10)]
    #扩充特征库
    addimgs = r"./addface"
    w = rec.addfeature(addimgs)
    print("特征库形状为{}".format(w.shape))
    # #与特征库作比较
    # img = r"test_img/1.jpg"
    ...
    # print(cls.item())
    # 比较两张
    # cosa = rec.comparetwoface(img,img)
    # print(cosa)
            ...
            for box in boxes:
                ...
                if (x2 - x1) > 100 and (y2 - y1) > 100:
                    ...
                    if cls[0][key].item() > 0.95:
                        draw.text((x1 + 10, y1 + 10), text=f'{name[key]}', fill='red',font=font)
                    else:
